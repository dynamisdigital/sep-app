...
function verificarOperacao(openapi, descriptor, operacao, resultado) {
  ...
  if (!doc) {
    resultado.falhas.push(
      `${operacao.id}: ${operacao.method.toUpperCase()} ${operacao.path} nao existe no OpenAPI`,
    );
  ...
  verificarCorpoDeErro(openapi, doc, operacao, descriptor, resultado);
}
...
function verificarParametros(params, operacao, descriptor, resultado) {
  ...
  for (const nome of query) {
    if (!nomesQueryDocumentados.includes(nome) && !pageable.includes(nome)) {
      resultado.falhas.push(
        `${operacao.id}: frontend envia query '${nome}' nao documentada no OpenAPI`,
      );
    }
  }
  if (pageable.length > 0 && !nomesQueryDocumentados.includes('pageable')) {
    resultado.falhas.push(
      `${operacao.id}: frontend pagina via page/size/sort mas OpenAPI nao documenta 'pageable'`,
    );
  }
  ...
  for (const nome of headers) {
    ...
    if (consumirGapDeHeader(descriptor, resultado, nome, operacao.id)) {
      resultado.lacunas.push(
        `${operacao.id}: header '${nome}' enviado pelo frontend nao documentado (lacuna conhecida)`,
      );
    } else {
      resultado.falhas.push(
        `${operacao.id}: frontend envia header '${nome}' nao documentado no OpenAPI`,
      );
    }
  }
}
...
function verificarParametrosDePath(params, operacao, resultado) {
  ...
  for (const [, nome] of operacao.path.matchAll(/\{(\w+)\}/g)) {
    ...
    if (!param) {
      resultado.falhas.push(
        `${operacao.id}: parametro de path '${nome}' do template nao documentado no OpenAPI`,
      );
    } else if (!param.required) {
      resultado.falhas.push(
        `${operacao.id}: parametro de path '${nome}' documentado como opcional no OpenAPI`,
      );
    }
  }
}
...
function verificarHeadersDaResposta(doc, operacao, descriptor, resultado) {
  ...
  if (Array.isArray(porStatus)) {
    resultado.falhas.push(
      `${operacao.id}: 'responseHeaders' e mapa por status ({ "200": [...] }), nao lista plana`,
    );
    return;
  }
  for (const [status, nomes] of Object.entries(porStatus)) {
    if (!Array.isArray(nomes)) {
      resultado.falhas.push(
        `${operacao.id}: 'responseHeaders[${status}]' deve ser lista de headers`,
      );
      continue;
    }
    ...
    if (!doc.responses?.[status]) {
      resultado.falhas.push(
        `${operacao.id}: 'responseHeaders' declara status ${status} nao documentado no OpenAPI`,
      );
      continue;
    }
...
function verificarStatusDeSucesso(doc, operacao, resultado) {
  for (const status of operacao.sucesso) {
    if (!doc.responses?.[String(status)]) {
      resultado.falhas.push(
        `${operacao.id}: status de sucesso ${status} tratado pelo frontend nao documentado no OpenAPI`,
      );
    }
  }
}
...
function verificarStatusDeErro(doc, operacao, resultado) {
  for (const status of operacao.erros ?? []) {
    if (!doc.responses?.[String(status)]) {
      resultado.falhas.push(
        `${operacao.id}: status de erro ${status} tratado pelo frontend nao documentado no OpenAPI`,
      );
    }
  }
}
...
function verificarCorpoDaRequisicao(openapi, doc, operacao, descriptor, resultado) {
  ...
  if (!conteudo?.schema) {
    resultado.falhas.push(
      `${operacao.id}: frontend envia body JSON mas OpenAPI nao documenta requestBody`,
    );
    return;
  }
  verificarExpectativa(
    openapi,
    descriptor,
    operacao.request,
    conteudo.schema,
...
function verificarCamposMultipart(openapi, doc, operacao, resultado) {
  const schemaMultipart = resolverRef(
    openapi,
    doc.requestBody?.content?.['multipart/form-data']?.schema,
  );
  const propriedades = Object.keys(schemaMultipart.properties ?? {});
  const nomesQueryDocumentados = (doc.parameters ?? [])
    .filter((p) => p.in === 'query')
    .map((p) => p.name);
...
function verificarCorpoDeErro(openapi, doc, operacao, descriptor, resultado) {
  ...
  if (Array.isArray(porStatus)) {
    resultado.falhas.push(
      `${operacao.id}: 'errorResponses' e mapa por status ({ "400": { "$type": ... } }), nao lista`,
    );
    return;
  }
  ...
  for (const [status, expectativa] of Object.entries(porStatus)) {
    ...
    if (
      !expectativa ||
      typeof expectativa !== 'object' ||
      !(expectativa.$type || expectativa.array)
    ) {
      resultado.falhas.push(
        `${operacao.id}: 'errorResponses[${status}]' deve ser { "$type": ... } ou { "array": ... }`,
      );
      continue;
    }
    if (!errosRamificados.includes(status)) {
      resultado.falhas.push(
        `${operacao.id}: 'errorResponses' declara status ${status} fora de 'erros' — so se declara corpo de status que a tela ramifica`,
      );
      continue;
    }
    ...
    if (!conteudo?.schema) {
      resultado.falhas.push(
        `${operacao.id}: resposta de erro ${status} sem schema JSON no OpenAPI`,
      );
      continue;
    }
    verificarExpectativa(
      ...
      expectativa,
      conteudo.schema,
      `${operacao.id}.errorResponses[${status}]`,
      resultado,
    );
  }
}
...
function verificarTipoNomeado(
  ...
  if (!tipo) {
    resultado.falhas.push(
      `${caminho}: tipo '${nomeTipo}' nao declarado em consumed-contracts.json`,
    );
    return;
  }
  ...
  for (const [nomeCampo, especificacao] of Object.entries(tipo.fields)) {
    ...
    if (!propriedade) {
      resultado.falhas.push(
        `${caminho}: campo '${nomeCampo}' de ${nomeTipo} nao existe no schema OpenAPI`,
      );
      continue;
    }
    ...
    if (consumirGapDeTipoDeCampo(descriptor, resultado, nomeTipo, nomeCampo)) {
      resultado.lacunas.push(
        `${caminho}.${nomeCampo}: tipo divergente do runtime documentado como lacuna conhecida`,
      );
      continue;
    }
...
function verificarCampo(
  ...
  if (especificacao.array) {
    if (tipoDoSchema(prop) !== 'array') {
      resultado.falhas.push(
        `${caminho}: frontend espera array, OpenAPI documenta '${tipoDoSchema(prop)}'`,
      );
      return;
    }
    verificarCampo(
...
function verificarTipoPrimitivo(tipoEsperado, prop, caminho, resultado) {
  ...
  if (!compativeis.includes(tipoOpenapi)) {
    resultado.falhas.push(
      `${caminho}: frontend espera '${tipoEsperado}', OpenAPI documenta '${tipoOpenapi}'`,
    );
  }
}
...
function verificarEnumSubset(
  ...
  for (const valor of valoresConsumidos) {
    if (!prop.enum.includes(valor)) {
      resultado.falhas.push(
        `${caminho}: frontend depende de '${valor}', ausente do enum documentado no OpenAPI`,
      );
    }
  }
}
...
function reportarEnumNaoPublicado(descriptor, valores, caminho, resultado, nomeTipo, nomeCampo) {
  if (consumirGapDeEnum(descriptor, resultado, nomeTipo, nomeCampo)) {
    ...
    resultado.falhas.push(
      `${caminho}: frontend espera enum ${JSON.stringify(valores)}, OpenAPI nao publica enum`,
    );
  }
}
...
function varrerGapsObsoletos(descriptor, resultado) {
  ...
  (descriptor.knownGaps ?? []).forEach((gap, indice) => {
    ...
    resultado.obsoletos.push(
      `knownGaps[${indice}] (${identificarGap(gap)}) nao e consumido por nenhuma operacao`,
    );
  });
}
...
async function carregarOpenapi(origem) {
  if (/^https?:\/\//.test(origem)) {
    ...
    if (!resposta.ok)
      throw new Error(`Falha ao baixar OpenAPI de ${origem}: HTTP ${resposta.status}`);
...
async function main() {
  ...
  const { falhas, lacunas, obsoletos, operacoesVerificadas } = verificarContratos(
    openapi,
    descriptor,
  );
